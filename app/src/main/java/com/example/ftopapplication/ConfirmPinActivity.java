...
public class ConfirmPinActivity extends AppCompatActivity implements NumberPadFragment.OnNumberPadClickListener {

    private List<View> pinDots = new ArrayList<>();
    private StringBuilder enteredPin = new StringBuilder();
    private String newPin;  // Mã PIN mới từ `SetUpNewPinActivity`
    ...
    private void confirmPin() {
        if (enteredPin.toString().equals(newPin)) {
            Toast.makeText(this, "Yay! Your new PIN code has been saved.", Toast.LENGTH_SHORT).show();
            finish();
        } else {
            Toast.makeText(this, "The pin you entered is wrong or setup failed. Try again", Toast.LENGTH_SHORT).show();
            enteredPin.setLength(0);  // Xóa mã PIN đã nhập nếu sai
            updatePinDisplay();
        }
    }
}
